...
// Gerado em: 2026-06-18T21:31:14.961Z
// ══════════════════════════════════════════
...
async function handleLogs(request, env, cors) {
  ...
  const logs  = await env.DB.prepare(
    `SELECT tipos_detectados, total_pii, chars_entrada, chars_saida, tempo_ms, ner_ok, criado_em
...
async function handleLogsExport(request, env, cors) {
  ...
  const header = "data,total_pii,chars_entrada,chars_saida,tempo_ms,ner_ok,tipos\n";
  const rows   = logs.results.map(r =>
    `"${r.criado_em}",${r.total_pii},${r.chars_entrada},${r.chars_saida},${r.tempo_ms},${r.ner_ok},"${r.tipos_detectados}"`
  ).join("\n");
...
async function enviarRelatorioDiario(env) {
  try {
    ...
    const dispNER     = total > 0 ? (((total - nerFalhas) / total) * 100).toFixed(1) : "100.0";
    const fbTotal     = feedbackHoje?.total || 0;
    const fbSim       = feedbackHoje?.util_sim || 0;
    const fbPct       = fbTotal > 0 ? Math.round((fbSim / fbTotal) * 100) : "—";

    await enviarTelegram(env,
      `📊 *MascaraAI — ${hoje}*\n\n` +
      `📈 *USO*\nScans hoje: ${total} | Mês: ${scansMes?.total || 0}\n` +
      `PII mascarados hoje: ${scansHoje?.pii_total || 0}\nScans zero PII: ${scansHoje?.zero_pii || 0} ⚠️\n\n` +
      `⚡ *PERFORMANCE*\nTempo médio: ${Math.round(scansHoje?.tempo_medio || 0)}ms\n` +
      `Tempo máximo: ${scansHoje?.tempo_max || 0}ms\nNER disponível: ${dispNER}%\n\n` +
      `🔍 *TOP TIPOS*\n${topTipos}\n\n` +
      `💬 *FEEDBACK*\nTotal: ${fbTotal} | ✅ ${fbSim} (${fbPct}%) | ❌ ${fbTotal - fbSim}\n\n` +
...
async function salvarLog(env, accountHash, resultado, charsEntrada, tempoTotal) {
  await env.DB.prepare(
    `INSERT INTO scan_logs (account_hash, tipos_detectados, total_pii, chars_entrada, chars_saida, tempo_ms, tempo_ner_ms, ner_ok, ner_fallback, pii_regex, pii_ner, criado_em)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,datetime('now'))`
  ).bind(
    accountHash,
